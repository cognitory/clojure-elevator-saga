                      ...
                      (if-let [elevator (->> (world-state :elevators)
                                             (filter (fn [elevator]
                                                       (and
                                                         ...
                                                         (let [target-direction (if (> (person :target-floor)
                                                                                       (-> person :location :floor))
                                                                                  :up
                                                                                  :down)]
                                                           (contains? (elevator :indicators) target-direction))
                                                         (< (->> people
                                                                 (filter (fn [person]
...
(defn- move-elevators [world-state elevator-commands]
  (update world-state :elevators
          (fn [elevators]
            (vec (for [elevator elevators]
                   (let [command (get elevator-commands (elevator :index) {})]
                     (case (command :action)
                       :up
...
(defn update-indicators [world-state elevator-commands]
  (update world-state :elevators
          (fn [elevators]
            (vec (for [elevator elevators]
                   (if-let [indicators (get-in elevator-commands [(elevator :index) :indicators])]
                     (assoc elevator :indicators indicators)
                     (assoc elevator :indicators #{})))))))


(defn- tick [world-state]
  (let [elevator-commands ((world-state :elevator-logic) (to-player-state world-state))]
    (-> world-state
        (move-elevators elevator-commands)
        (update-indicators elevator-commands)
